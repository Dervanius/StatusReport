...
namespace StatusReport.Controllers
{
    public class EcomController : Controller
    {
        ...
        public async Task<IActionResult> UploadExcelFile(FileUploadViewModel model, string criteria, string status)
        {
            ...
            else if (criteria == "external" && status == "temporary")
            {
                var jsonList = JsonConvert.SerializeObject(barcodes);

                var reportResults = await _reportServices.GetTemporaryReport(jsonList, false);

                return await GenerateTempExcelFile(reportResults);
            }
        ...
        private async Task<IActionResult> GenerateTempExcelFile(IEnumerable<dynamic> reportResults)
        {
            using (var package = new ExcelPackage())
            {
                var worksheet = package.Workbook.Worksheets.Add("Report");
                ...
                // Adding header row
                worksheet.Cells[1, 1].Value = "External Number";
                worksheet.Cells[1, 2].Value = "Status";
                worksheet.Cells[1, 3].Value = "Status Date";
                worksheet.Cells[1, 4].Value = "Status Time";
                ...
                foreach (var result in reportResults)
                {
                    worksheet.Cells[row, 1].Value = result.ExternalNumber;
                    worksheet.Cells[row, 2].Value = result.Status;
                    worksheet.Cells[row, 3].Value = result.EventDateOnly;
